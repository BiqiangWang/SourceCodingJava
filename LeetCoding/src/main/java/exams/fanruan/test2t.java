...
public class test2t {
    ...
    public static void main(String[] args) {
        ...
        int k = 6;
        int[] ans = new test2t().minNumber(nums1, nums2, k);
    ...
    private int[] dfs(int i, int j, int[] path, int cur) {
        if (cur == k) {
            for (int t = 0; t < k; t++) {
                if (path[t] < ans[t]) {
                    ans = update(path, ans);
        ...
        if (cur + (nums1.length - i) + (nums2.length - j) < k) return null;
        if (cur == 0) {
